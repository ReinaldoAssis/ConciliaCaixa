...
class CaixaRestauranteRepository:
    def __init__(self, path: str | Path | None = None) -> None:
        self.path = Path(path) if path else db_path()
        try:
            from tinydb import Query, TinyDB
        ...
        self._query = Query()
        self._db = TinyDB(self.path, ensure_ascii=False, indent=2)
        self._caixas = self._db.table("caixas_restaurante")
    ...
    def _normalize(caixa: dict) -> dict:
        normalized = dict(caixa)
        normalized.setdefault("status", "rascunho")
        normalized.setdefault("turno", None)
        normalized.setdefault("categorias", empty_categories_restaurante())
        normalized["categorias"] = normalize_categories_restaurante(normalized["categorias"])
        normalized.setdefault("contagens_dinheiro", [])
        normalized.setdefault("observacoes", "")
        normalized.setdefault("lancamentos_avulsos", [])
        avulsos = normalized.get("lancamentos_avulsos") or []
        ...
        normalized["diferenca_total"] = diferenca
        normalized["dinheiro_real"] = round(dinheiro_real, 2)
        return normalized
